-- =====================================================
-- Database Migration for Fan Engagement Features
-- =====================================================
-- Run this script to add author_id to polls and create quiz tables
-- =====================================================

-- Add author_id column to polls table if it doesn't exist
ALTER TABLE `polls` 
ADD COLUMN IF NOT EXISTS `author_id` int DEFAULT NULL AFTER `description`,
ADD KEY IF NOT EXISTS `author_id` (`author_id`);

-- Add foreign key constraint if it doesn't exist
-- Note: MySQL doesn't support IF NOT EXISTS for constraints, so this may need manual check
-- ALTER TABLE `polls` 
-- ADD CONSTRAINT IF NOT EXISTS `polls_ibfk_2` FOREIGN KEY (`author_id`) REFERENCES `users` (`id`) ON DELETE SET NULL;
