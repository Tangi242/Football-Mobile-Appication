-- =====================================================
-- Safe Database Migration for Fan Engagement Features
-- =====================================================
-- Run this script to add author_id to polls and create quiz tables
-- This version checks for existing columns/tables before creating
-- =====================================================

-- Add author_id column to polls table (only if it doesn't exist)
SET @dbname = DATABASE();
SET @tablename = 'polls';
...
SET @preparedStatement = (SELECT IF(
  ...
  CONCAT('ALTER TABLE `', @tablename, '` ADD COLUMN `', @columnname, '` int DEFAULT NULL AFTER `description`, ADD KEY `author_id` (`author_id`)')
));
...
-- Note: MySQL doesn't have a simple way to check for existing constraints
-- You may need to manually verify and add this if it doesn't exist:
-- ALTER TABLE `polls` ADD CONSTRAINT `polls_ibfk_2` FOREIGN KEY (`author_id`) REFERENCES `users` (`id`) ON DELETE SET NULL;

-- =====================================================
-- Create Quiz Tables (IF NOT EXISTS is already in CREATE TABLE)
-- =====================================================
